...
**Oxirgi yangilanish:** 2026-06-10 (IG Highlights + 3a TUGADI: avatar crop + rang palitralari) · **Faol branch:** `dev` · **Repo:** https://github.com/Burxoniddin/MyLink (public)
...
- [x] **3a · Shablonlar + ranglar + avatar** — ✅ **Shablonlar qismi tayyor**: `Business.template` (classic + 5 soha shabloni: restoran/moda/klinika/avto/fitnes, migration `0012`), serializer (read+write) + public payload, admin. Frontend: `components/templates/` (`ProfileTemplate` + `templates.css` + `TemplatePicker` + `templateMeta` + `lib/linkIcons`), `LandingPage` shablon bo'yicha shoxlanadi (light/dark per-page, ulashish, analitika tracking saqlangan), editor **Sozlash** tabida shablon tanlash, shrift'lar `index.html`da. Shablonlar **hamma tarifga ochiq** (gate yo'q). uz/ru/en. Testlar: +4 (jami 68). ✅ **Avatar kesish:** logo yuklash/drop qilinganda `react-easy-crop` bilan **kvadrat crop modali** (`components/LogoCropper.jsx`) — pan/zoom, dumaloq ko'rinish, canvas→PNG File; `BusinessDetail` logo oqimiga ulandi; uz/ru/en `crop.*`. ✅ **Rang sozlash:** `Business.theme` palitra (default/ocean/forest/noir/rose/sunset, migration `0013`) + `color_edit` gate (serializer update'da, Oddiy/Pro). Frontend: `lib/palettes.js` + `components/ThemePicker.jsx` (Sozlash tabida, faqat `classic` shablonda, Free→Pro upsell); classic `LandingPage` palitra **bg** + `LinkButton accent` (sektor shablonlar o'z rangida). uz/ru/en `theme.*`. Testlar: +3 (jami **71**). **3a TO'LIQ TUGADI.**
